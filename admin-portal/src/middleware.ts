...
// Sign-in is public — everything else requires platform_admin role
...
export default clerkMiddleware(async (auth, request) => {
  ...
  // Enforce authentication — redirect to /sign-in if not signed in
  await auth.protect();
  ...
  try {
    ...
    // Backend error — deny access and redirect to sign-in
    return NextResponse.redirect(new URL("/sign-in", request.url));
  } catch {
    // Backend unreachable — redirect to sign-in (fail secure)
    return NextResponse.redirect(new URL("/sign-in", request.url));
  }
});
